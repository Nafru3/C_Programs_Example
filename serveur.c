...
Objectif : Implémentation du serveur avec la gestion de la réponse contenant message et calcul
...
int recois_envoie_message(int client_socket_fd, char *data)
{
  ...
  if (strncmp(data,"message :",9) == 0) { // Le n dans strncmp c'est pour pouvoir rentrer la taille
  ...
  } else if (strncmp(data,"calcul :",8) == 0) {
  ...
  char sortie[1000];
  sprintf(sortie, "calcul : %d", result);
